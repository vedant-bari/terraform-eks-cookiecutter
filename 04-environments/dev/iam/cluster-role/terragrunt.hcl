...
terraform {
  source = "../../../../02-modules/03-iam/cluster-role"
}
...
remote_state {
  ...
  config = {
    ...
    key          = "${local.env.locals.backend.key}/iam/cluster-role/terraform.tfstate"
...
inputs = {
  role_name = local.env.locals.iam.cluster_role_name
